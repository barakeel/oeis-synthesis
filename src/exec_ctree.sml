structure exec_ctree :> exec_ctree =
struct

open HolKernel boolLib aiLib kernel bloom exec_prim
val ERR = mk_HOL_ERR "exec_ctree"
...
val ctdefault = ctzero
val default_entry = (ctdefault, ctdefault)
...
fun mk_binf opf fl = case fl of
    ...
    in
      case t1 of
        CLeaf x1 => CLeaf (opf x1 x2) 
      | CNode1 (x1,st) => CNode1 (opf x1 x2, st)
      | CNode2 (x1,st1,st2) => CNode2 (opf x1 x2, st1, st2)
    end
    )
...
val mult_f = mk_binf cmult
val divi_f = mk_binf cdivi
val modu_f = mk_binf cmodu
...
fun loop2m_f_aux () = 
  let 
    ...
    val mem = (mema,memn)
  in 
    fn (f1,f2,n,x1,x2) => helperm mem f1 f2 (mk_bound n) x1 x2
  end
...
fun loopm_f_aux () =
  let
    ...
    val memn = ref 0
    val mem = (mema,memn)
    fun f2 (x1,x2) = ctincr x2
  in
    fn (f1,n,x1) => helperm mem f1 f2 (mk_bound n) x1 ctone
  end
...
fun loop_f_aux (f1,n,x1) = 
  helper f1 (fn (x1,x2) => ctincr x2) (mk_bound n) x1 ctone
...
fun compr_next f x =
  if cleq0 (root (f (x,ctzero))) then x else compr_next f (ctincr x)

(* i and x starts at -1 *)           
fun compr_loop mem f n i x =
  if i >= n then x else 
  let 
    val newx = compr_next f (ctincr x)
    val newi = i+1
  in
    store_mem (mem,newi,(newx,ctdefault)); 
    compr_loop mem f n newi newx
  end
   
val comprstart = ctmone
...
fun compr_f fl = case fl of
    ...
    let val f1' = compr_wrap f1 in
      (fn x => (checktimer (); f1' (mk_bound (f2 x))))
    end
  | _ => raise ERR "compr_f" ""
...
fun mk_exec_onev p = 
  let val exec = mk_exec p in 
    (fn x => mk_return (exec (mk_ctree (mk_complex x),ctzero)))
  end
