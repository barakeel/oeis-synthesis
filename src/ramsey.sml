...
(* first_clique k f n (1.6 ^ k)     win if n >= 64 *)  
...
(* -------------------------------------------------------------------------
   Incrementally adding vertices (alternative)
   + enforcing monotonicity of the first clique (optional)
   reverse order of vertices to avoid duplicated work
   ------------------------------------------------------------------------- *)
...
val a = [(1, 1), (2, 3), (3, 6), (4, 8), (5, 20), (6, 27), (7, 43), (8, 67),
    (9, 91)]: (int * int) list
val b =
...
fun number_of_ones i = 
  if i < 2 then i else i mod 2 + number_of_ones (i div 2);
fun f (i,j) = j*j mod (2*(i+j)+1) > i+j;
fun f (i,j) = (IntInf.log2 (IntInf.fromInt j - IntInf.fromInt i)) mod 2 <= 0;
fun f(i,j) = number_of_ones (j - i) mod 2 <= 0;
fun f (i,j) = j*j mod (2*(i+j)+1) < i+j;
val f' = mk_cache 256 f;
val l = (first_clique f' 8);

val l = all_clique 4 f' (List.tabulate (20,I));
