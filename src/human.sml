...
fun human vn prog = 
  let 
    fun heads s = if !use_list then s^".hd()"
  ...
  in
    case prog of
    Ins (0,[]) => if !use_list then "A(["^ its 0 ^ "])" else its 0
  | Ins (1,[]) => if !use_list then "A(["^ its 1 ^ "])" else its 1
  | Ins (2,[]) => if !use_list then "A(["^ its 2 ^ "])" else its 2
  ...
  | Ins (8,[p1,p2,p3]) => 
    "(" ^ h p2 ^ " if " ^ heads (h p1) ^ " <= 0 else " ^ h p3 ^ ")"
  | Ins (9,[p1,p2,p3]) => 
    let fun f wn =
...
(* Todo: supports programs that depend on y *)
...
(* -------------------------------------------------------------------------
   Printing to Python with base type: list of integers. 
   ------------------------------------------------------------------------- *)
