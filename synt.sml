...
fun score_match gseq (seq,anum) = 
  let 
    val anumi = string_to_int (tl_string anum)
    val anumr = Real.fromInt anumi / 10000000.0
...
fun score_matchl gseq =
  let val l = List.mapPartial (score_match gseq) aol in
    first_n 3 (dict_sort compare_rmax l) 
  end

fun print_match (matchn,contb,shiftn,anum) =
  print_endline
    ("Matches " ^ 
    "<a href=https://oeis.org/search?q=" ^ anum ^ ">" ^ anum ^ 
    "</a> (s_n) " ^
    "from s_" ^ its shiftn ^ " to s_" ^ its (shiftn + matchn) ^
    (if contb then "." else " and deviates after that point.")
    )
