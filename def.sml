...
(* -------------------------------------------------------------------------
   Definitions are programs with holes
   ------------------------------------------------------------------------- *)
...
(* -------------------------------------------------------------------------
   Create the program building operator from a definition with holes
   ------------------------------------------------------------------------- *)
...
(* -------------------------------------------------------------------------
   Import defintions from file def
   ------------------------------------------------------------------------- *)
...
fun expand_anode cutoff (attop as (ANode (def,plil,_))) =
  ...
  in
  if n <= 0 orelse prog_size def > 25 then attop else  
  let
...
fun b_def cutoff sol =
  let 
    ...
    val defl3 = dict_sort compare_imax defl2;
