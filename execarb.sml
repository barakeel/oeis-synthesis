...
fun coverp_oeis p = cover_oeis (mk_execarb p)
fun coverp_target p target = cover_target (mk_execarb p) target
...
fun add_ocache p = 
  ...
  let val l1 = penumt timeincr p 1000 in
    if length l1 >= 1000 then () else
    let val l2 = penumt 0.001 p 1000 in
