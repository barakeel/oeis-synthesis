  ...
  let val fl = map mk_execarb_aux pl in
    ...
    if id = 12 then
      let val v = dfind (hd pl) (!ccache) handle NotFound =>
        raise ERR "mk_execarb_aux" (raw_prog p)
...
fun mk_execarb p = 
  let val _ = init_execarb p in (fn x => mk_execarb_aux p x) end
  
fun coverp_oeis p = 
  let val r = cover_oeis (mk_execarb p) in reset_ccache (); r end

fun coverp_target p target = 
  let val r = cover_target (mk_execarb p) target in reset_ccache (); r end
